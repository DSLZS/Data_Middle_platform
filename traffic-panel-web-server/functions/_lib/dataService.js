import seedTrips from "../../mock/trips.mock.json";
...
function jitterTrip(baseTrip, tag, sequence, mode = "normal", hourSlot = null) {
  ...
  for (let i = 0; i < pointCount; i += 1) {
    ...
    lat.push(
      Number(
        (baseTrip.lat[srcIdx] + latJitter + moveFactor * rand(...moveRange)).toFixed(
          6
        )
      )
    );
    lon.push(
      Number(
        (baseTrip.lon[srcIdx] + lonJitter + moveFactor * rand(...moveRangeLon)).toFixed(
          6
        )
      )
    );
...
function buildSyntheticTrips(sourceTrips, count, tag) {
  return Array.from({ length: count }, (_, i) => {
    const base = sourceTrips[i % sourceTrips.length];
...
function resetSimulationData() {
  const sourceTrips = Array.isArray(seedTrips) ? seedTrips : [];
  ...
  liveTrips = buildSyntheticTrips(sourceTrips, INITIAL_TRIPS, "init");
  pendingTrips = buildSyntheticTrips(sourceTrips, PENDING_TRIPS, "pending");
}

function syncTripsToNow() {
  const elapsedMs = Math.max(0, Date.now() - bootTimestamp);
  const shouldAppend = Math.min(
    PENDING_TRIPS,
    Math.floor(elapsedMs / WRITE_INTERVAL_MS)
  );
  const currentWritten = Math.max(0, liveTrips.length - INITIAL_TRIPS);
  const delta = shouldAppend - currentWritten;

  if (delta > 0) {
    liveTrips.push(...pendingTrips.splice(0, delta));
  }
}
...
export function getTrips(limit = 10) {
  ...
  return trips
    ...
    .map((trip, idx) => {
      ...
      const avgSpeedKmh =
        durationHour > 0 ? Number((distanceKm / durationHour).toFixed(2)) : 0;
      return {
