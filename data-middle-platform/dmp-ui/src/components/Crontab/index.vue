...
const { hideComponent = [], expression = '' } = defineProps<Props>()
const tabTitles = ref(['秒', '分钟', '小时', '日', '月', '周', '年'])
// const tabActive = ref(0)
// const hideComponent = ref([])
// const expression = ref('')
const crontabValueObj = ref<CronType>({
  second: '*',
  min: '*',
  hour: '*',
  day: '*',
...
function shouldHide(key: string) {
  return !(hideComponent && hideComponent.includes(key))
}
function resolveExp() {
  // 反解析 表达式
  if (expression) {
