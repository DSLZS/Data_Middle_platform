...
import { user } from '@/views/pageName.ts'
...
const postOptions = ref<UserPost[]>([])
/** 查询部门下拉树结构 */
...
const tableSelected = ref<UserRow[]>([])
const tableListener = {
  selectionChange: (selected: UserRow[]) => {
...
const editCallBack = (_item: UserRow, type: any, res: UserGetInfo) => {
...
const handleStatusChange = async (row: UserRow) => {
...
/** 文件上传成功处理 */
const handleFileSuccess = ({ response }: { response: ResBase }) => {
...
<template>
  <div class="default-main page">
    ...
    >
      ...
      <template #sexSlot="{ backData }">
        <DictTag :value="backData.sex" :options="sys_user_sex"></DictTag>
      </template>
